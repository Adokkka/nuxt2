...
export default {
  ...
  mounted() {
    //const userid = "7c3496bd-c860-4348-ae5f-6c14468b37a4";
    this.getolinfo(this.olduid);
    this.fetchUsers();

    this.fetchExpenditure();
    this.olduid = this.$route.params.uid;
  ...
  methods: {
    async getolinfo(olduid) {
      ...
      this.detailedUserData = JSON.parse(response.data.area);
      this.tableData = JSON.parse(response.data.supplier);
    ...
    async sendRequest(status) {
      ...
      const payload = {
        ...
        //  org_name: this.detailedUserData.org_name,
        ...
        //  org_bin: this.detailedUserData.org_bin,
      ...
      try {
        const response = await axios.put(
          `http://127.0.0.1:8000/expenditure/update/${this.olduid}/`,
          payload
        );
